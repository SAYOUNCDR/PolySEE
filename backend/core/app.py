...
except ImportError:  # fallback for legacy langchain versions
...
def retrieve_documents(retriever, query: str):
    """Return relevant docs across LangChain releases."""
    ...
    raise AttributeError("Unsupported retriever interface")
...
async def chat_with_assistant(
    ...
    docs = retrieve_documents(retriever, payload.message)
    approved = [
...
def test_retrieval(request: ChatRequest):
    ...
    docs = retrieve_documents(retriever, request.message)
    return {
        "status": "success",
